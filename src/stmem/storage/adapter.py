...
from stmem.storage.base import VectorStoreBase
from stmem.utils.utils import serialize_datetime, get_current_datetime
...
class StorageAdapter:
    ...
    def _to_epoch_seconds(value: Any) -> Optional[int]:
        ...
        try:
            ...
            if isinstance(value, str) and value.strip():
                return int(_dt.fromisoformat(value.replace("Z", "+00:00")).timestamp())
        except Exception:
            return None
        return None
    ...
    def add_memory(self, memory_data: Dict[str, Any]) -> int:
        ...
        created_ts = self._to_epoch_seconds(memory_data.get("created_at"))
        updated_ts = self._to_epoch_seconds(memory_data.get("updated_at"))
        if created_ts is not None:
            payload["created_at_ts"] = created_ts
        if updated_ts is not None:
            payload["updated_at_ts"] = updated_ts
        ...
        excluded_fields = ["id", "content", "data", "user_id", "run_id", "metadata", "filters", 
    ...
    def search_memories(
        ...
        # Merge user_id/run_id into filters to ensure consistency
        ...
        for result in results:
            ...
            promoted_payload_keys = ["user_id", "run_id", "actor_id", "role"]
    ...
    def update_memory(
        ...
        existing_memory_dict = self.get_memory(memory_id, user_id)
        ...
        created_ts = updated_payload.get("created_at_ts")
        if created_ts is None:
            computed_created_ts = self._to_epoch_seconds(updated_payload.get("created_at"))
            if computed_created_ts is not None:
                updated_payload["created_at_ts"] = computed_created_ts

        computed_updated_ts = self._to_epoch_seconds(updated_payload.get("updated_at"))
        if computed_updated_ts is not None:
            updated_payload["updated_at_ts"] = computed_updated_ts
    ...
    def get_all_memories(
        ...
        for result in raw_results:
            ...
            memory = {
                ...
                "should_forget": payload.get("should_forget"),
    ...
    def clear_memories(
        ...
        while True:
            ...
            batch = self.get_all_memories(user_id, run_id, limit=batch_size, offset=deleted_count)
    ...
    async def get_all_memories_async(
        ...
        return await asyncio.to_thread(
            self.get_all_memories, user_id, run_id, limit, offset, sort_by, order, filters
        )
    ...
    async def clear_memories_async(
        ...
        import asyncio
        return await asyncio.to_thread(self.clear_memories, user_id, run_id)
    ...
    async def search_memories_async(
        ...
        import asyncio
        return await asyncio.to_thread(self.search_memories, query_embedding, user_id, run_id, filters, limit, query)
    ...
    async def update_memory_async(
        ...
        import asyncio
        return await asyncio.to_thread(self.update_memory, memory_id, update_data, user_id)
...
class SubStorageAdapter(StorageAdapter):
    ...
    def __init__(self, vector_store: VectorStoreBase, embedding_service=None, sparse_embedder_service=None):
        ...
        from stmem.storage.migration_manager import SubStoreMigrationManager
    ...
    def migrate_to_sub_store(
        ...
        try:
            ...
            while True:
                results = self.vector_store.list(
                    filters=routing_filter or None,
                    limit=batch_size,
                    offset=offset,
                    order_by="id",
                    order="asc",
                ...
                for item in batch:
                    ...
                    if record_id is None or not payload:
                        continue

                    payload_dict = payload.copy() if isinstance(payload, dict) else {}
                    payload_dict["id"] = int(record_id)
                    content = payload_dict.get("data") or payload_dict.get("content") or ""
                    if not content:
                        continue

                    vector = sub_embedding_service.embed(content, memory_action="add")
                    target_store.insert([vector], [payload_dict], ids=[record_id])
                    migrated_count += 1
