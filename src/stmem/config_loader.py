"""
Configuration loader for stmem
...
try:
    ...
    from stmem.utils.pydantic_compat import AliasChoices, BaseModel, ConfigDict, Field
...
from stmem.integrations.embeddings.config.providers import CustomEmbeddingConfig
from stmem.integrations.embeddings.config.sparse_base import BaseSparseEmbedderConfig
from stmem.integrations.llm.config.base import BaseLLMConfig
from stmem.settings import _DEFAULT_ENV_FILE, settings_config
...
class TelemetrySettings(_BasePowermemSettings):
    ...
    endpoint: str = Field(
        default="https://telemetry.stmem.ai",
...
class LoggingSettings(_BasePowermemSettings):
    ...
    file: str = Field(default="./logs/stmem.log")
...
class DatabaseSettings(_BasePowermemSettings):
    ...
    def to_config(self) -> Dict[str, Any]:
        ...
        """
        from stmem.storage.config.base import BaseVectorStoreConfig
...
class RerankerSettings(_BasePowermemSettings):
    ...
    def to_config(self) -> Dict[str, Any]:
        ...
        """
        from stmem.integrations.rerank.config.base import BaseRerankConfig
...
class GraphStoreSettings(_BasePowermemSettings):
    ...
    def to_config(
        ...
        from stmem.storage.config.base import BaseGraphStoreConfig
...
class PowermemSettings:
    ...
    def to_config(self) -> Dict[str, Any]:
        ...
        llm_provider_explicit = "provider" in getattr(self.llm, "model_fields_set", set())
        embedder_provider_explicit = "provider" in getattr(self.embedder, "model_fields_set", set())

        llm_api_key = (config.get("llm", {}).get("config") or {}).get("api_key")
        embedder_api_key = (config.get("embedder", {}).get("config") or {}).get("api_key")

        if not llm_provider_explicit and not llm_api_key:
            config["llm"] = {"provider": "mock", "config": {"model": "mock"}}
            intelligent_cfg = config.get("intelligent_memory") or {}
            if isinstance(intelligent_cfg, dict):
                intelligent_cfg = {
                    **intelligent_cfg,
                    "enabled": False,
                ...
                intelligent_cfg = {"enabled": False, "fallback_to_simple_add": True}
            config["intelligent_memory"] = intelligent_cfg

        if not embedder_provider_explicit and not embedder_api_key:
            dims = (
                (config.get("embedder", {}).get("config") or {}).get("embedding_dims")
                or (config.get("vector_store", {}).get("config") or {}).get("embedding_model_dims")
                or 1536
            )
            config["embedder"] = {"provider": "mock", "config": {"embedding_dims": dims}}
...
def load_config_from_env() -> Dict[str, Any]:
    ...
    It automatically detects the database provider (sqlite, postgres) and builds
    ...
    Example:
        ```python
        from dotenv import load_dotenv
        from stmem.config_loader import load_config_from_env
...
def create_config(
    ...
    Args:
        database_provider: Database provider ('sqlite', 'postgres')
    ...
    Example:
        ```python
        from stmem.config_loader import create_config
        from stmem import Memory
...
def validate_config(config: Dict[str, Any]) -> bool:
    ...
    Example:
        ```python
        from stmem.config_loader import load_config_from_env, validate_config
...
def auto_config() -> Dict[str, Any]:
    ...
    Example:
        ...
        from stmem import create_memory
