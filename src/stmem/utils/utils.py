...
def get_current_datetime() -> datetime:
    ...
    This function is used throughout stmem to get the current time in the
    ...
    Note:
        All timestamps in stmem (created_at, updated_at, etc.) are generated
...
def parse_json_from_text(text: str, expected_type: type = dict) -> Optional[Any]:
    ...
    def extract_balanced(s: str, open_ch: str, close_ch: str) -> List[str]:
        segments: List[str] = []
        depth = 0
        start = None
        in_str = False
    ...
    segments = (
        extract_balanced(text, "{", "}") if expected_type == dict else extract_balanced(text, "[", "]")
    )
    ...
    logger.debug(f"Failed to parse JSON from text, expected type: {expected_type}, text: {preview}")
    return None
