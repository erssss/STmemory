...
class MemorySearch:
    def __init__(self, output_path, top_k=10, filter_memories=False, is_graph=False):
        ...
        if not openai_api_key:
            base = str(self.openai_base_url or "")
        ...
        base = str(self.openai_base_url or "")
        self._use_ollama_native = ("127.0.0.1:11434" in base) or ("localhost:11434" in base)
        self.top_k = top_k
    ...
    def answer_question(self, speaker_1_user_id, speaker_2_user_id, question, answer, category):
        ...
        t1 = time.time()
        try:
            if bool(getattr(self, "_use_ollama_native", False)):
                base = str(self.openai_base_url or "").rstrip("/")
                if base.endswith("/v1"):
                    base = base[:-3].rstrip("/")
                r = requests.post(
                    f"{base}/api/chat",
                    json={"model": self.model, "stream": False, "messages": [{"role": "system", "content": answer_prompt}]},
                    headers={"Content-Type": "application/json"},
                    timeout=60,
                )
                r.raise_for_status()
                data = r.json() if r.text else {}
                response_content = (((data.get("message") or {}) or {}).get("content") or "")
            else:
