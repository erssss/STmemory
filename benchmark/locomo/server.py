    ...
    async def search(self, user_id: str, query: str) -> Dict[str, Any]:
        ...
        for entry in memories:
            ...
            if entry.metadata and entry.metadata.get("timestamp") is not None:
                ts_str = str(entry.metadata.get("timestamp"))
            score = plugin.ranker.compute_spatiotemporal_score(query, entry, now, entry.layer).score
            results.append(
                {
                    "memory": entry.query if entry.response == "" else f"{entry.query}\n{entry.response}",
                    "metadata": {"timestamp": ts_str},
                    "score": float(score),
                }
            )
...
def create_app(service: _STMemoryService, enable_mock_openai: bool) -> web.Application:
    ...
    async def search(request: web.Request) -> web.Response:
        ...
        res = await service.search(str(user_id), str(query))
        return web.json_response(res)
