...
from benchmark.soda.reporting import (
    LocomoThresholds,
    check_thresholds,
...
def _soda_dir(repo_root: str) -> str:
    soda_dir = os.path.abspath(os.path.dirname(__file__))
    dataset_path = os.path.join(soda_dir, "dataset", "soda10.json")
    if not os.path.isfile(dataset_path):
        raise FileNotFoundError(f"Missing SODA dataset: {dataset_path}")
    return soda_dir


def _run_soda_script(soda_dir: str, args: List[str], env: Dict[str, str]) -> None:
    subprocess.run([sys.executable, *args], cwd=soda_dir, env=env, check=True)


def _append_generate_scores(soda_dir: str, results_dir: str, evaluation_txt_path: str, env: Dict[str, str]) -> None:
    p = subprocess.run(
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run SODA benchmark integrated with STmemory")
    parser.add_argument("--config", default=None)
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--port", type=int, default=8000)
    parser.add_argument("--output-dir", default=None)
    parser.add_argument("--dataset", default="benchmark/soda/dataset/soda10.json", help="SODA dataset path")
    ...
    soda_dir = _soda_dir(repo_root)
    api_base_url = f"http://{args.host}:{args.port}"
    dataset_path = args.dataset or os.path.join(soda_dir, "dataset", "soda10.json")
    ...
    if not output_dir:
        ...
        output_dir = os.path.join(repo_root, "benchmark", "results", "soda", stamp)
    ...
    try:
        ...
        env["SODA_LLM_PROVIDER"] = str(args.llm_provider)
        ...
        _run_soda_script(soda_dir, run_args_add, env=env)
        ...
        _run_soda_script(soda_dir, run_args_search, env=env)
        ...
        if not os.path.exists(evaluation_txt_path):
            with open(evaluation_txt_path, "w", encoding="utf-8") as f:
                f.write("")
        _append_generate_scores(soda_dir, output_dir, evaluation_txt_path, env=env)
        ...
        if bool(args.run_full) and subset_indices:
            ...
            env_full = env.copy()
            env_full.pop("SODA_SUBSET_INDICES", None)
            ...
            _run_soda_script(soda_dir, run_args_add_full, env=env_full)
            ...
            _run_soda_script(soda_dir, run_args_search_full, env=env_full)
            ...
            if not os.path.exists(evaluation_txt_path_full):
                with open(evaluation_txt_path_full, "w", encoding="utf-8") as f:
                    f.write("")
            _append_generate_scores(soda_dir, full_dir, evaluation_txt_path_full, env=env_full)
